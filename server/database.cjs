...
async function createSchema() {
  ...
  const statements = [
    ...
    `CREATE TABLE IF NOT EXISTS convites_colaboradores (
      id ${id}, empresa_id INTEGER NOT NULL, criado_por INTEGER,
      token TEXT UNIQUE NOT NULL, nome TEXT DEFAULT '', email TEXT DEFAULT '',
      perfil TEXT DEFAULT 'marinheiro', papel TEXT DEFAULT 'membro', cargo TEXT DEFAULT '',
      usado INTEGER DEFAULT 0, usado_por INTEGER, expira_em TIMESTAMP, criado_em TIMESTAMP DEFAULT ${now}
    )`,
...
async function migrateCollaboratorInvites() {
  const id = engine === "postgresql"
    ? "INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY"
    : "INTEGER PRIMARY KEY AUTOINCREMENT";

  await execute(`CREATE TABLE IF NOT EXISTS convites_colaboradores (
    id ${id}, empresa_id INTEGER NOT NULL, criado_por INTEGER,
    token TEXT UNIQUE NOT NULL, nome TEXT DEFAULT '', email TEXT DEFAULT '',
    perfil TEXT DEFAULT 'marinheiro', papel TEXT DEFAULT 'membro', cargo TEXT DEFAULT '',
    usado INTEGER DEFAULT 0, usado_por INTEGER, expira_em TIMESTAMP, criado_em TIMESTAMP DEFAULT CURRENT_TIMESTAMP
  )`);
  if (engine === "postgresql") {
    await execute("CREATE INDEX IF NOT EXISTS idx_convites_empresa ON convites_colaboradores (empresa_id, criado_em)");
    await execute("CREATE INDEX IF NOT EXISTS idx_convites_token ON convites_colaboradores (token)");
  }
}
...
async function initialize() {
  ...
  const collaboratorInvitesMigration = await get(
    "SELECT version FROM schema_migrations WHERE version = ?",
    ["013_collaborator_invites"],
  );
  if (!collaboratorInvitesMigration) {
    await migrateCollaboratorInvites();
    await run(
      "INSERT INTO schema_migrations (version) VALUES (?)",
      ["013_collaborator_invites"],
    );
